...
import type { L2DEventMap, Options, ParamInfo } from './types'
...
class L2D extends Emitter<L2DEventMap> {
	...
	private _state: ModelState = { l2d2Model: null }
	...
	private _on(type: string, handler: (detail: any) => void) {
		window.addEventListener(type, (e: Event) => {
			...
			if (detail?.canvas === this.canvas) handler(detail)
		})
	}

	/**
	 * 加载并渲染一个 Cubism 2 Live2D 模型。
	 * 加载完成后触发 `loaded` 事件。
	...
	resize() {
		if (this._state.l2d2Model) this._state.l2d2Model.resize()
	}

	/** 暂停渲染循环，适用于页面隐藏或组件不可见时。 */
	pause() {
		this._state.l2d2Model?.pause()
	}

	/** 恢复渲染循环。 */
	resume() {
		this._state.l2d2Model?.resume()
	}
	...
	setVolume(volume: number) {
		if (this._state.l2d2Model) this._state.l2d2Model.setVolume(volume)
	}
	...
	setScale(scale: number) {
		if (this._state.l2d2Model) this._state.l2d2Model.setScale(scale)
	}
	...
	setPosition(x: number, y: number) {
		if (this._state.l2d2Model) this._state.l2d2Model.setPosition(x, y)
	}
	...
	setParams(params: Record<string, number>) {
		if (this._state.l2d2Model) this._state.l2d2Model.setParams(params)
	}
	 ...
	 */
	getParams(): ParamInfo[] {
		if (this._state.l2d2Model) return this._state.l2d2Model.getParams()
		return []
	}
	...
	getHitAreaBounds(): Array<{ name: string; x: number; y: number; w: number; h: number }> {
		const raw = this._state.l2d2Model ? this._state.l2d2Model.getHitAreaBounds() : []
